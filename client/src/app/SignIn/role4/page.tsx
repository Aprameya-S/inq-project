...
const Page = () => {
  ...
  const handleSubmit = (e:any) => {
    e.preventDefault()
    try {
      axios.post('http://localhost:5174/api/auth/loginRole4',form)
      .then((res) => {
        if(res.status===200){
          push('/Role4')
        }
  ...
  return (
    <div className="grid items-center h-screen">
        ...
        <div className="">
          ...
          <h2 className='text-[18px] font-medium text-blue-600'>Role 4</h2>
        </div>
        <form onSubmit={handleSubmit}>
          ...
          <Link href={'SignIn/role4/forgotPassword'} className='text-blue-600 text-[12px]'>Forgot password?</Link><br/>
